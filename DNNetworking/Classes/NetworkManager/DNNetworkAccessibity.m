...
NSString * const DNNetworkAccessibityChangedNotification = @"DNNetworkAccessibityChangedNotification";

typedef NS_ENUM(NSInteger, DNNetworkType) {
    DNNetworkTypeUnknown ,
    DNNetworkTypeOffline ,
    DNNetworkTypeWiFi    ,
    DNNetworkTypeCellularData ,
};

@interface DNNetworkAccessibity (){
    SCNetworkReachabilityRef _reachabilityRef;
    CTCellularData *_cellularData;
    NSMutableArray *_becomeActiveCallbacks;
    DNNetworkAccessibleStatus _previousState;
...
@implementation DNNetworkAccessibity
...
+ (DNNetworkAccessibity *)sharedInstance {
    static DNNetworkAccessibity * instance = nil;
...
- (void)monitorNetworkAccessibleStateWithCompletionBlock:(void (^)(DNNetworkAccessibleStatus))block {
...
- (void)setupNetworkAccessibity {
    ...
    BOOL firstRun = ({
        static NSString * RUN_FLAG = @"DNNetworkAccessibityRunFlag";
...
- (void)cleanNetworkAccessibity {
    ...
    _previousState = DNNetworkChecking;
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    DNNetworkAccessibity *networkAccessibity = (__bridge DNNetworkAccessibity *)info;
    if (![networkAccessibity isKindOfClass: [DNNetworkAccessibity class]]) {
        return;
    }
...
- (void)startCheck {
    ...
    if ([self currentReachable]) {
        /* 先用 currentReachable 判断，若返回的为 YES 则说明：
         ...
         此时是有网络访问权限的，直接返回 DNNetworkAccessible
         **/
        return [self notiWithAccessibleState:DNNetworkAccessible];
    }
    ...
    switch (state) {
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(DNNetworkType type) {
                /*  若用户是通过蜂窝数据 或 WLAN 上网，走到这里来 说明权限被关闭**/
                
                if (type == DNNetworkTypeCellularData || type == DNNetworkTypeWiFi) {
                    [self notiWithAccessibleState:DNNetworkRestricted];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:DNNetworkUnknown];
                }
            }];
...
- (void)getCurrentNetworkType:(void(^)(DNNetworkType))block {
    if ([self isWiFiEnable]) {
        return block(DNNetworkTypeWiFi);
    }
    DNNetworkType type = [self getNetworkTypeFromStatusBar];
    if (type == DNNetworkTypeWiFi) { // 这时候从状态栏拿到的是 Wi-Fi 说明状态栏没有刷新，延迟一会再获取
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (DNNetworkType)getNetworkTypeFromStatusBar {
    NSInteger type = 0;
    @try {
...
- (void)notiWithAccessibleState:(DNNetworkAccessibleStatus)state {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DNNetworkAccessibityChangedNotification object:nil];
